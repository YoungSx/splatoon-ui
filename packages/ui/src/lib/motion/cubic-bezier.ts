...
export type CubicBezierControlPoints = readonly [x1: number, y1: number, x2: number, y2: number]
...
export function createCubicBezierSampler(
  ...
  return (progress: number) => {
    ...
    const curvature =
      (yCurvature * safeXSlope - ySlope * xCurvature) / (safeXSlope * safeXSlope * safeXSlope)
