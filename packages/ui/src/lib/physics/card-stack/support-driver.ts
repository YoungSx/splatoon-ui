...
import { createCubicBezierSampler, splatoonEasings, type SplatoonEasingName } from '@/lib/motion'
...
function sampleCurveMotion(
  ...
  const carryB =
    (state.initialAccelerationPxPerSecondSquared * durationSeconds * durationSeconds) / 2
  ...
  const carryPosition =
    carryA * progress + carryB * progress * progress + carryC * progress * progress * progress
  ...
  return {
    ...
    accelerationPxPerSecondSquared:
      (deltaPx * normalized.curvature + carryCurvature) / (durationSeconds * durationSeconds),
    done: false,
  }
}
...
function sampleSpringMotion(
  ...
  if (dampingRatio < 1 - 1e-6) {
    ...
    velocity =
      exponential *
      (velocity0 * cos -
        ((decay * velocity0 + naturalFrequency * naturalFrequency * displacement0) /
          dampedFrequency) *
          sin)
    ...
    displacement =
      coefficient1 * Math.exp(lambda1 * elapsedSeconds) +
      coefficient2 * Math.exp(lambda2 * elapsedSeconds)
    velocity =
